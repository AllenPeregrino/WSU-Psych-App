...
def create_report():
    ...
    for i in range (35, 36):
        ...
        df = Graph_Generator.pd.read_csv('Capstone Working Survey.csv')#, skiprows=40)#, nrows=100)
        idxS = 0-i
        #print(df)
...
def rs_page(pdf, rs):
    ...
    total = "Rejection Sensitivity Score: " + str(round(rs, 2))
...
def csip_bar_graphs(my_path, pdf, data, names, titles):
    pdf.add_page()
    PDF_Generator.section_headers(pdf, 'Circumplex scales of interpersonal problems')
    counter = 0
    # loop through all the items in the dictionary
    dataCopy = data.copy()
    ...
    for key, value in dataCopy.items():
        # If counter == 2 we need to create a new page
        if counter == 2:
            Graph_Generator.create_csip_bargraph(pdf, my_path, (counter*96), value, names, key, titles.pop(0))
            pdf.image(my_path + "/images/RSSM_Scaling.png", 118, counter*96+86, WIDTH/2-20)
            pdf.add_page()
            PDF_Generator.section_headers(pdf, 'Circumplex scales of interpersonal problems')
            counter = 0
            ...
            if counter != 0:
                ...
                Graph_Generator.create_csip_bargraph(pdf, my_path, 8, value, names, key, titles.pop(0))
                pdf.image(my_path + "/images/RSSM_Scaling.png", 118, 94, WIDTH/2-20)
                counter += 1
